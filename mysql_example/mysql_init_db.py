...
Tables created:
* raw_dedupe_table - raw import of entire CSV file
...
c.execute("DROP TABLE IF EXISTS raw_dedupe_table")
...
c.execute("CREATE TABLE raw_dedupe_table "
...
c.execute("LOAD DATA LOCAL INFILE %s INTO TABLE raw_dedupe_table "
...
c.execute("DELETE FROM raw_dedupe_table WHERE LENGTH(date_recieved) < 10")

# set empty, non-zero, strings in date columns to null
c.execute("UPDATE raw_dedupe_table SET report_period_begin = NULL WHERE LENGTH(report_period_begin) < 10")
c.execute("UPDATE raw_dedupe_table SET report_period_end = NULL WHERE LENGTH(report_period_end) < 10")

#committee ID is requred. Remove the 2 rows that don't have it.
c.execute("DELETE FROM raw_dedupe_table WHERE committee_id=''");
...
# way.
c.execute("DELETE FROM raw_dedupe_table WHERE LENGTH( committee_id ) > 9")
conn.commit()
...
c.execute("INSERT IGNORE INTO recipients "
          "SELECT DISTINCT committee_id, committee_name FROM raw_dedupe_table")
conn.commit()
...
c.execute("INSERT INTO contributions "
          ...
          "FROM raw_dedupe_table JOIN donors ON "
          "donors.first_name = TRIM(raw_dedupe_table.first_name) AND "
          "donors.last_name = TRIM(raw_dedupe_table.last_name) AND "
          "donors.address_1 = TRIM(raw_dedupe_table.address_1) AND "
          "donors.address_2 = TRIM(raw_dedupe_table.address_2) AND "
          "donors.city = TRIM(raw_dedupe_table.city) AND "
          "donors.state = TRIM(raw_dedupe_table.state) AND "
          "donors.employer = TRIM(raw_dedupe_table.employer) AND "
          "donors.occupation = TRIM(raw_dedupe_table.occupation) AND "
          "donors.zip = TRIM(raw_dedupe_table.zip)")
conn.commit()
